...
import type { QuoteState, PlanType, Segmento, QuoteMode } from './types'
...
export type QuoteAction =
  ...
  // Modo
  | { type: 'SET_MODO'; payload: QuoteMode }
  // Plan seleccionado
  | { type: 'SET_PLAN_SELECCIONADO'; payload: PlanType | null }
  // A la carta - Telemed
...
export function quoteReducer(state: QuoteState, action: QuoteAction): QuoteState {
  switch (action.type) {
    ...
    case 'SET_POBLACION':
      return { ...state, contrato: { ...state.contrato, poblacion: Math.max(0, action.payload) } }
    ...
    case 'SET_MODO': {
      ...
      if (action.payload === 'SOLO_PAQUETES') {
        return { ...state, modo: action.payload, planSeleccionado: null }
      }
    ...
    case 'SET_PLAN_SELECCIONADO':
      ...
      if (state.modo === 'SOLO_PAQUETES') {
        return { ...state, planSeleccionado: null }
      }
    ...
    case 'SET_ALC_ZENTIS_USUARIOS':
      return { ...state, alaCartaZentis: { ...state.alaCartaZentis, usuariosActivos: Math.max(0, action.payload) } }
    ...
    case 'SET_ALC_ZENTIS_IMAGENES_USUARIOS':
      return { ...state, alaCartaZentis: { ...state.alaCartaZentis, zentisImagenesUsuarios: Math.max(0, action.payload) } }
    ...
    case 'RESET_STATE':
      return { ...structuredClone(INITIAL_STATE), quoteId: crypto.randomUUID(), quoteName: 'Nueva Cotización' }
    case 'SET_QUOTE_NAME':
